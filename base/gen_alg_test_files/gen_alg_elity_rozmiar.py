...
def genetic_algorithm_schedule(population_size=200, generations=10000, elitism_sizes=None):
    creator.create("FitnessMin", base.Fitness, weights=(-1.0,))
    creator.create("Individual", list, fitness=creator.FitnessMin)

    selection_method = tools.selBest
    ...
    if elitism_sizes is None:
        elitism_sizes = [0, 1, 2, 5, 8]  # Default elitism sizes to test
    ...
    for elitism_size in elitism_sizes:
        ...
        for run in range(5):  # Run the algorithm 5 times
            ...
            for gen in range(generations):
                ...
                if best_ind.fitness.values[0] == 0:
                    print(f"Solution found with elitism size {elitism_size} in generation {gen} on run {run + 1}")
                    break

            print(f"Elitism size {elitism_size}, Run {run + 1}: Total generations run: {len(conflicts_over_time)}")
        ...
        aggregated_results[f"Elitism {elitism_size}"] = averaged_results

    # Plotting results
    plt.figure(figsize=(12, 8))
    for method_name, conflicts in aggregated_results.items():
        plt.plot(range(len(conflicts)), conflicts, label=method_name)

    plt.xlabel("Pokolenia")
    plt.ylabel("Średnia liczba konfliktów")
    plt.title("Algorytm genetyczny: wpływ rozmiaru elity na liczbę konfliktów")
    plt.legend(title="Rozmiar elity")
    plt.grid()
    plt.savefig("porownanie_rozmiaru_elity.png")
