...
def genetic_algorithm_schedule(
    ...
    selection_methods = {
        "Turniej (rozmiar 3)": (tools.selTournament, {"tournsize": 3}),
        "Turniej (rozmiar 5)": (tools.selTournament, {"tournsize": 5}),
        "Ruletka": (tools.selRoulette, {}),
        "Rankingowa": (tools.selStochasticUniversalSampling, {}),
        "Losowa": (tools.selRandom, {}),
        "Elitarna (5%)": (tools.selBest, {"k": int(population_size * 0.05)}),
        "Elitarna (10%)": (tools.selBest, {"k": int(population_size * 0.1)}),
        "Elitarna (20%)": (tools.selBest, {"k": int(population_size * 0.2)}),
        "Elitarna (30%)": (tools.selBest, {"k": int(population_size * 0.3)}),
    }
    ...
    for method_name, (method, method_args) in selection_methods.items():
        method_results = []

        for run in range(5):  # Powtarzamy algorytm 5 razy dla średniej
            if method == tools.selBest:
                def custom_select(population, k):
                    return method(population, k=method_args["k"])
                toolbox.register("select", custom_select)
            else:
                toolbox.register("select", method, **method_args)
        ...
        results[method_name] = averaged_results
    ...
    plt.title("Porównanie metod selekcji")
    plt.legend()
    plt.grid()
    plt.savefig("selection3_methods_comparison.png")
