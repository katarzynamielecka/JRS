...
def genetic_algorithm_schedule(
    population_size=100, generations=10000, mutation_probabilities=None
):
    ...
    if mutation_probabilities is None:
        mutation_probabilities = [0.1, 0.3, 0.5, 0.6, 0.7, 0.8, 1.0]
    ...
    for mutation_prob in mutation_probabilities:
        ...
        for run in range(5):  # Run the algorithm 5 times
            ...
            for gen in range(generations):
                elites = tools.selBest(population, 1)  # Elita jako lista
                offspring = tools.selTournament(population, len(population) - len(elites), tournsize=3)
        ...
        aggregated_results[f"Mutation {mutation_prob}"] = averaged_results

    plt.figure(figsize=(12, 8))
    for mutation_prob, conflicts in aggregated_results.items():
        plt.plot(range(len(conflicts)), conflicts, label=mutation_prob)

    plt.xlabel("Pokolenie")
    plt.ylabel("Średnia liczba konfliktów")
    plt.title("Wpływ prawdopodobieństwa mutacji na liczbę konfliktów")
    plt.legend(title="Prawdopodobieństwo mutacji")
    plt.grid()
    plt.savefig("mutation_probability_comparison.png")
